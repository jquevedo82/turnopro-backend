...
**Última actualización:** Marzo 2026 — Sesión 18
...
| Secretaria no ve servicios/clientes del profesional | GET /services y GET /clients necesitan aceptar ?professionalId — ver services.controller.ts y clients.controller.ts actualizados |
...
- [x] **Panel secretaria — páginas completas**
  - `SecretaryNewAppointment` — formulario completo con servicios y slots del profesional activo, guard si no hay profesional seleccionado
  - `SecretaryClientsPage` — lista de clientes del profesional activo, guard si no hay profesional seleccionado
  - `services.controller.ts` — GET /services acepta ?professionalId para secretaria
  - `clients.controller.ts` — GET /clients acepta ?professionalId para secretaria
  - `appointments.api.ts` — agrega createForProfessional con ?professionalId query param
  - `useAppointments.ts` — agrega useCreateAppointmentForProfessional(professionalId)
...
**Frontend + Backend**
```
feat(secretary): completar panel secretaria con nueva cita y clientes

- SecretaryNewAppointment.tsx: formulario completo con servicios y slots del profesional activo
- SecretaryClientsPage.tsx: lista de clientes del profesional activo
- Ambas páginas con guard si no hay profesional seleccionado
- appointments.api.ts: agrega createForProfessional con ?professionalId query param
- useAppointments.ts: agrega useCreateAppointmentForProfessional(professionalId)
- services.controller.ts: GET /services acepta ?professionalId para secretaria
- clients.controller.ts: GET /clients acepta ?professionalId para secretaria
...
**Docs**
```
docs: Manual de Usuario v4 — circuitos superadmin, profesional, secretaria y paciente
```
