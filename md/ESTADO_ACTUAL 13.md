...
**Última actualización:** Marzo 2026 — Sesión 13
...
- Foto de perfil del profesional — StorageService dual (Cloudinary en prod / disco local en VPS). Endpoint POST /professionals/avatar. Campo `avatar` en la entidad. Elimina avatar anterior antes de subir el nuevo.
...
- Foto de perfil en ProfilePage — componente AvatarUpload con preview instantánea, spinner y validación (JPG/PNG/WebP, máx 2MB)
- Página pública muestra avatar del profesional — círculo con foto o inicial como fallback. Tamaño 168px mobile / 192px desktop.
...
- [x] **Foto de perfil del profesional** — StorageService dual Cloudinary/Local. POST /professionals/avatar. AvatarUpload en ProfilePage con preview + spinner. PublicPage muestra avatar o inicial como fallback. Archivos: storage.service.ts, storage.module.ts, professionals.module.ts, professionals.controller.ts, update-profile.dto.ts, ProfilePage.tsx, PublicPage.tsx
...
```
feat(professionals): foto de perfil con StorageService dual Cloudinary/Local

- storage.service.ts: upload/delete con estrategia Cloudinary (prod) o Local (VPS)
- storage.module.ts: módulo exportable
- professionals.module.ts: agrega MulterModule (memoryStorage) y StorageModule
- professionals.controller.ts: POST /professionals/avatar, borra avatar anterior, updateAvatar()
- update-profile.dto.ts: agrega campo avatar
- ProfilePage.tsx: componente AvatarUpload con preview instantánea y spinner
- PublicPage.tsx: muestra avatar del profesional, fallback a inicial
```
